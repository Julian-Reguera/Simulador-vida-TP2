...
public class Vector2D {
	...
	/*
	 * //devuelve la distancia entre este y that teniendo en cuenta que si sales por
	 * arriba apareces por abajo y igual en el eje x public double
	 * distanceToInMap(Vector2D that,double width, double heigth) { Vector2D
	 * distancia1 = minus(that); double x = distancia1.getX(); double y =
	 * distancia1.getY(); x = Math.abs(x); y = Math.abs(y); return (new
	 * Vector2D(x>width/2 ?width-x:x,y>heigth/2 ?heigth-y:y)).magnitude(); }
	 */
...
//		r._y = matrix[1][0] * x + matrix[1][1] * y;
	...
	public static Vector2D randomPoint(double width, double height) {
		double x = Utils._rand.nextDouble(width);
		double y = Utils._rand.nextDouble(height);
		return new Vector2D(x, y);
	}

	public Vector2D ajustarPosicion(double width, double height) {
		double x = _x, y = _y;
