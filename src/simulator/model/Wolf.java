...
public class Wolf extends Animal{
	...
	public void update(double dt) {
		if(_state != State.DEAD)
		{
			...
			if(get_energy() == 0 || _age > MAX_AGE) _state = State.DEAD;
			if(_state != State.DEAD) add_energy(_region_mngr.get_food(this, dt));
		}
	}
	...
	private void actualizaHunger(double dt)
	{
		...
		if(_hunt_target == null || _hunt_target.get_state() == State.DEAD || _hunt_target.get_position().distanceTo(_pos) > _sight_range)
		{
			_hunt_target = _hunting_strategy.select(this, _region_mngr.get_animals_in_range(this, a -> a.get_diet() == Diet.HERBIVORE));
		}
		
		if(_hunt_target == null) actualizaNormal(dt);
		else {
	...
	private void actualizaMate(double dt)
	{
		if (_mate_target == null || _mate_target.get_state() == State.DEAD
			...
			_mate_target = _mate_strategy.select(this,_region_mngr.get_animals_in_range(this, a -> a.get_genetic_code().equals(_genetic_code)));
		}
		
		if(_mate_target == null) actualizaNormal(dt);
		else {
	...
	private void cambioEstado()
	{
		switch(_state) {
		case NORMAL:
			if(get_energy()<ENERGY_HUNGUER) _state = State.HUNGER;
			else if(get_desire()>DESIRE_MATE) _state = State.MATE;
			break;
		case HUNGER:
			if(get_energy()>ENERGY_HUNGUER)
			{
				if(get_desire()<DESIRE_MATE) _state = State.NORMAL;
				else _state = State.MATE;
			}
			break;
		case MATE:
			if(get_energy()<ENERGY_HUNGUER) _state = State.HUNGER;
			else if(get_desire()<DESIRE_MATE) _state = State.NORMAL;
			break;
		default:
			break;
		}
